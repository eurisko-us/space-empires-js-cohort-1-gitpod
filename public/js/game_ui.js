...
socket.on("gameState", function(data) { updateBoard(data.gameState); });
...
function updateBoard(gameState) {
  ...
  if (boardTable) { document.getElementById("board-div").removeChild(boardTable); }
  ...
  for (let y = 0; y < gameState.boardSize; y++) {
    ...
    for (let x = 0; x < gameState.boardSize; x++) {
      ...
      if (hexAttributes) {
        ...
        if (hexAttributes["planet"] == null && hexAttributes["asteroid"] == null && hexAttributes["units"].length == 0) { hex.style.backgroundColor = "grey"; } // empty
          ...
          if (currentPlanet["colony"] != null && currentPlanet["colony"] != true) {

            let playerIndex = currentPlanet["colony"]["playerIndex"];
            let player = gameState["players"][playerIndex]
            planetColor = player["playerColor"];
            hex.innerHTML = currentPlanet["colony"]["type"];
            
          }

          if (hexAttributes["units"].length > 0) { hex.innerHTML += "<br>Ships: " + hexAttributes["units"].length; }
          ...
          for (let i in playerColors) { coloring = coloring.concat(`, ${playerColors[i]} ${size*i}%, ${playerColors[i]} ${size*(i+1)}%`); }
